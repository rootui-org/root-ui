# @rootui/drawer

[![Github repository](https://img.shields.io/badge/github-repo-blue?logo=github)](https://github.com/rootui-org/root-ui) [![Npm monthly downloads](https://img.shields.io/npm/dm/@rootui/drawer)](https://www.npmjs.com/package/@rootui/drawer) [![Npm monthly downloads](https://img.shields.io/badge/live-demo-green)](https://root-ui.com/primitives/drawer) [![Root UI X profile](https://img.shields.io/twitter/follow/rootui_)](https://x.com/rootui_)

A panel that slides in from the edge of the screen (also called Sheet component).

 - 📖 Docs: [Root UI drawer docs](https://root-ui.com/primitives/drawer)
 - 📄 Source: [Github repository](https://github.com/rootui-org/root-ui)
 - 🐛 Report an issue: [Open an issue](#)
 - 📦 Package: [@rootui/drawer](https://www.npmjs.com/package/@rootui/drawer)
 - 🔗 Reference: [React Aria docs](https://react-aria.adobe.com/Modal)

## 🧬 Anatomy
```tsx
<Drawer>
  <Drawer.Trigger>
    Click me to open drawer
  </Drawer.Trigger>
  <Drawer.Content>
    Hello, I'm the content!
  </Drawer.Content>
</Drawer>
```
## 🛠️ Installation
### 1. 📌 Prerequisite
#### 1.1. ⚙️ Install dependecies
Check the [getting started guide](https://root-ui.com/docs/getting-started) for more info.
If you haven't installed the dependecies yet, then run this command:
```bash
npm install react-aria-components tw-animate-css
# or
yarn add react-aria-components tw-animate-css
# or
pnpm add react-aria-components tw-animate-css
# or
bun add react-aria-components tw-animate-css
```
#### 1.2. 🎨 Choose a theme
You need to pick a theme from the [official Root UI website](https://root-ui.com/playground).


### 2. 🍉 Install component
#### Option 1: 📦 Package
```bash
npm install @rootui/drawer
# or
yarn add @rootui/drawer
# or
pnpm add @rootui/drawer
# or
bun add @rootui/drawer
```

#### Option 2: ⬛ Shadcn CLI
```bash
npx shadcn@latest add @rootui/drawer
# or
yarn shadcn@latest add @rootui/drawer
# or
pnpm dlx shadcn@latest add @rootui/drawer
# or
bunx --bun shadcn@latest add @rootui/drawer
```

#### Option 3: 🏗️ Manual
Check the [Root UI drawer docs](https://root-ui.com/docs/primitives/drawer#installation).

## 💻 Usage
```tsx
import { Drawer } from "@rootui/drawer";

export default function Page() {
  return (
    <Drawer>
      <Drawer.Trigger>
        This is the trigger
      </Drawer.Trigger>
      <Drawer.Content>
        This is the content
      </Drawer.Content>
    </Drawer>
  )
}
```
## 📃 API Reference
Check the [Root UI drawer docs](https://root-ui.com/docs/primitives/drawer#api-reference).

## 🤝 Contributing
Contributions are welcome! If you have suggestions for improvements or encounter any issues, please open an issue or submit a pull request on the  [GitHub repository](#).

Steps to Contribute:

1.  Fork the repository. 🍴
2.  Create a new branch for your feature or bugfix. 🌿
3.  Commit your changes. 💾
4.  Push your branch and submit a pull request. 🚀

## 📜 License
This project is licensed under the MIT License. See the  [LICENSE](https://github.com/rootui-org/root-ui/blob/main/LICENSE.md)  file for details.